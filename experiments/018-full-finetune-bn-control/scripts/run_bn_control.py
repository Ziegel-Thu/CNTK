#!/usr/bin/env python3
"""BatchNorm/stat-mode controls for full ResNet18 fine-tuning."""
...
EXP_DIR = ROOT / "experiments" / "018-full-finetune-bn-control"
VARIANTS = [
    {"name": "layer4_base", "base_regime": "finetune_layer4", "bn_mode": "original"},
    {"name": "all_bn_train", "base_regime": "finetune_all", "bn_mode": "original"},
    {"name": "all_bn_eval", "base_regime": "finetune_all", "bn_mode": "all_eval"},
    {"name": "all_layer4_bn_train", "base_regime": "finetune_all", "bn_mode": "layer4_train"},
]
...
def is_batchnorm(exp013, module) -> bool:
    return isinstance(
        module,
        (
            exp013.nn.BatchNorm1d,
            exp013.nn.BatchNorm2d,
            exp013.nn.BatchNorm3d,
        ),
    )


def set_bn_eval(exp013, module) -> None:
    for child in module.modules():
        if is_batchnorm(exp013, child):
            child.eval()


def set_bn_train(exp013, module) -> None:
    for child in module.modules():
        if is_batchnorm(exp013, child):
            child.train()


def make_epoch_mode(exp013, bn_mode: str):
    original = exp013.set_epoch_train_mode

    def set_epoch_train_mode(model, regime: str) -> None:
        if bn_mode == "original":
            original(model, regime)
            return
        ...
        model.train()
        if bn_mode == "all_eval":
            set_bn_eval(exp013, model.backbone)
            model.head.train()
            return
        if bn_mode == "layer4_train":
            set_bn_eval(exp013, model.backbone)
            set_bn_train(exp013, model.backbone.layer4)
            model.head.train()
            return
        raise ValueError(f"unknown bn_mode: {bn_mode}")

    return set_epoch_train_mode
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        for task_idx, task in enumerate(tasks):
            for variant_idx, spec in enumerate(VARIANTS):
                ...
                exp013.set_epoch_train_mode = make_epoch_mode(exp013, spec["bn_mode"])
                try:
                    trace = exp013.train_regime(task, spec["base_regime"], args, device, seed=train_seed)
                finally:
    ...
    plot_results(finals)
    command = "python3 experiments/018-full-finetune-bn-control/scripts/run_bn_control.py"
    if args.quick:
        command += " --quick"
    else:
