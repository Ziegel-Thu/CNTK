#!/usr/bin/env python3
"""ViT-B/16 fine-tuning metric dynamics without BatchNorm."""
...
EXP_DIR = ROOT / "experiments" / "020-vit-finetune-metric-dynamics"
VARIANTS = [
...
def set_trainable(model: ViTMetricModel, regime: str) -> None:
    ...
    if regime == "finetune_lastblock":
        for param in model.backbone.encoder.layers[-1].parameters():
            param.requires_grad = True
...
def make_optimizer(model: ViTMetricModel, regime: str, args: argparse.Namespace) -> torch.optim.Optimizer:
    if regime == "frozen_head":
        return torch.optim.AdamW(model.head.parameters(), lr=args.head_lr, weight_decay=args.weight_decay)
    if regime == "finetune_lastblock":
        return torch.optim.AdamW(
            [
                {"params": model.backbone.encoder.layers[-1].parameters(), "lr": args.block_lr},
                {"params": model.backbone.encoder.ln.parameters(), "lr": args.block_lr},
...
def set_epoch_train_mode(model: ViTMetricModel, regime: str) -> None:
    model.train()
    if regime == "frozen_head":
        model.backbone.eval()
        model.head.train()
...
def main() -> None:
    ...
    for seed in args.seeds:
        ...
        for task_idx, task in enumerate(tasks):
            for variant_idx, spec in enumerate(VARIANTS):
                ...
                model = ViTMetricModel(num_classes=len(task["classes"])).to(device)
    ...
    plot_results(finals)
    command = "python3 experiments/020-vit-finetune-metric-dynamics/scripts/run_vit_dynamics.py"
    if args.quick:
        command += " --quick"
    else:
