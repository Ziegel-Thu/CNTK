#!/usr/bin/env python3
"""Frozen pretrained representation sweep for experiment 010."""
...
EXP_DIR = ROOT / "experiments" / "010-pretrained-fixed-representation-sweep"
...
def main() -> None:
    ...
    parser.add_argument("--binary-n-per-class", type=int, default=80)
    parser.add_argument("--multi-n-per-class", type=int, default=40)
    ...
    random_resnet = make_resnet_feature_extractor(pretrained=False, device=device)
    ...
    for task in tasks:
        ...
        train_random = extract_resnet_features(random_resnet, task["train_x"], device=device, batch_size=args.batch_size)
        test_random = extract_resnet_features(random_resnet, task["test_x"], device=device, batch_size=args.batch_size)
        rows.append(diagnostics(task, "random_resnet18", train_random, test_random, ridge=args.ridge))
    ...
    plot_results(rows)
    command = "python3 experiments/010-pretrained-fixed-representation-sweep/scripts/run_pretrained.py"
