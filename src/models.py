...
class ClassifierMLP(nn.Module):
    """Small ReLU MLP with a multiclass linear head."""

    def __init__(self, input_dim: int, num_classes: int, width: int = 128, depth: int = 2) -> None:
        super().__init__()
        if depth < 1:
            raise ValueError("depth must be at least 1")
...
class ClassifierSmallCNN(nn.Module):
    """Small CNN with a multiclass linear head."""

    def __init__(self, num_classes: int, width: int = 32, feature_dim: int = 128, in_channels: int = 3) -> None:
        super().__init__()
        self.feature_net = nn.Sequential(
